...
# 注意: Android APK 不在此 CI 自动打包 (CF Pages 容器无 Android SDK)。
#       APK 由本地手动 `flutter build apk --release --split-per-abi` 生成,
#       将 app-arm64-v8a-release.apk 重命名为 web/downloads/app-release.apk 后提交,
#       本脚本仅负责把它随 Web 产物一起部署到下载页。
